...
import { ListTodo, Plus, X, CheckCircle2, Circle, Clock, Tag, Send, ChevronDown, ChevronUp, Trash2, Loader2 } from "lucide-react";
...
const FORM_VAZIO = { titulo: "", descricao: "", tipo: "acao" as TipoTarefa, mentorada_nome: "", data_entrega: "" };
...
function isConcluida(status: string) {
  return status.toLowerCase() === "concluida" || status.toLowerCase() === "concluído" || status === "Concluída";
}
  ...
  async function enviar() {
    ...
    const res = await fetch("/api/tarefas", {
      ...
      body: JSON.stringify({
        ...
        tipo: form.tipo,
        mentorada_nome: form.mentorada_nome || null,
    ...
    <div style={{ maxWidth: 860, margin: "0 auto" }}>
      ...
      {mostrarForm && (
        <div className="card" style={{ padding: "20px 22px", marginBottom: 24, border: "1px solid var(--gold-border)", background: "linear-gradient(135deg,#111 0%,#130f04 100%)" }}>
          <div className="flex items-center justify-between" style={{ marginBottom: 16 }}>
            <p style={{ fontSize: 13, fontWeight: 700, color: "var(--gold)", margin: 0, textTransform: "uppercase", letterSpacing: "0.06em" }}>Nova Tarefa</p>
          ...
          <div style={{ display: "grid", gap: 12 }}>
            <input value={form.titulo} onChange={(e) => setForm({ ...form, titulo: e.target.value })} style={{ padding: "10px 12px", fontSize: 13, width: "100%" }} placeholder="O que você quer que ela faça?" />
            <textarea value={form.descricao} onChange={(e) => setForm({ ...form, descricao: e.target.value })} style={{ padding: "10px 12px", fontSize: 13, width: "100%", minHeight: 70, resize: "vertical" }} placeholder="Detalhe como ela deve executar..." />
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(160px, 1fr))", gap: 8 }}>
              ...
              <div>
                ...
                <select value={form.mentorada_nome} onChange={(e) => setForm({ ...form, mentorada_nome: e.target.value })} style={{ padding: "9px 12px", fontSize: 13, width: "100%" }}>
              ...
              <div>
                ...
                <input type="date" value={form.data_entrega} onChange={(e) => setForm({ ...form, data_entrega: e.target.value })} style={{ padding: "9px 12px", fontSize: 13, width: "100%" }} />
              </div>
            </div>
            <div className="flex items-center gap-3">
              <button className="btn-gold flex items-center gap-2" onClick={enviar} disabled={salvando || !form.titulo.trim()}>
                {salvando ? <Loader2 size={13} style={{ animation: "spin 1s linear infinite" }} /> : <Send size={13} />}
                {salvando ? "Enviando..." : "Enviar Tarefa"}
              </button>
              <button onClick={() => { setMostrarForm(false); setForm(FORM_VAZIO); }} style={{ background: "none", border: "1px solid var(--border)", color: "var(--text-muted)", padding: "8px 16px", borderRadius: 8, fontSize: 13, cursor: "pointer" }}>Cancelar</button>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="flex flex-col gap-2">
        {filtradas.map((t) => {
          ...
          return (
            <div key={t.id} className="card" style={{ padding: "13px 16px", opacity: concluida ? 0.65 : 1, border: vencida ? "1px solid rgba(252,165,165,0.25)" : "1px solid var(--border)", transition: "opacity 0.2s" }}>
              <div className="flex items-start gap-3">
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div className="flex items-center gap-3" style={{ marginTop: 6, fontSize: 10, color: "var(--text-muted)" }}>
                    ...
                    {t.data_entrega && <span style={{ color: vencida ? "#fca5a5" : "var(--text-muted)" }}><Tag size={9} style={{ display: "inline", marginRight: 3 }} />Prazo: {formatarData(t.data_entrega)}</span>}
                  </div>
                </div>
