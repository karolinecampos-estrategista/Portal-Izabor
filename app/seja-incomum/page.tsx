...
import {
  Crown, Plus, X, MessageCircle, Instagram, Trash2,
  CheckCircle2, Clock, ChevronDown, ChevronUp, Pencil, Loader2, Mail,
} from "lucide-react";
import ProdutosAcesso from "@/components/ProdutosAcesso";

type StatusAcesso = "ativo" | "inativo" | "pendente";
...
const statusCfg: Record<StatusAcesso, { label: string; cor: string; bg: string }> = {
  ...
  inativo:  { label: "Inativa",  cor: "#6b7280", bg: "rgba(107,114,128,0.12)" },
};
...
const FORM_VAZIO: Omit<Compradora, "id"> = {
  ...
  status_acesso: "ativo", progresso: 0, notas: null,
...
export default function SejaIncomumCompradores() {
  ...
  async function adicionar() {
    ...
    setSalvando(true);
    const res = await fetch("/api/seja-incomum", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ ...form, dataCompra: form.data_compra, statusAcesso: form.status_acesso, statusPagamento: form.status_pagamento, formaPagamento: form.forma_pagamento, produtos: formProdutos }),
    });
  ...
  async function salvarEdicao() {
    ...
    const res = await fetch("/api/seja-incomum", {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ ...editando, dataCompra: editando.data_compra, statusAcesso: editando.status_acesso, statusPagamento: editando.status_pagamento, formaPagamento: editando.forma_pagamento }),
    });
    const atualizado = await res.json();
    if (atualizado?.id) {
      setCompradoras(prev => prev.map(c => c.id === atualizado.id ? atualizado : c));
  ...
  async function excluir(id: string) {
    if (!confirm("Excluir esta aluna?")) return;
    await fetch(`/api/seja-incomum?id=${id}`, { method: "DELETE" });
    ...
    <div style={{ maxWidth: 960, margin: "0 auto" }}>
      ...
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", flexWrap: "wrap", gap: 12, marginBottom: 24 }}>
        <div>
          <div className="flex items-center gap-2" style={{ marginBottom: 4 }}>
            ...
            <span style={{ fontSize: 11, color: "var(--gold)", fontWeight: 600, letterSpacing: "0.08em", textTransform: "uppercase" }}>Seja Incomum</span>
          </div>
          <h1 style={{ fontSize: 20, fontWeight: 700, margin: 0 }}>Seja Incomum — Alunas</h1>
          <p style={{ color: "var(--text-muted)", fontSize: 13, marginTop: 4 }}>{ativas} aluna{ativas !== 1 ? "s" : ""} ativa{ativas !== 1 ? "s" : ""} na mentoria.</p>
        </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 12, marginBottom: 20 }}>
        {[
          ...
          { label: "Inativas", value: compradoras.filter(c => c.status_acesso === "inativo").length, cor: "#6b7280" },
        ].map(s => (
      ...
      {showForm && (
        <div className="card" style={{ padding: 20, marginBottom: 20, border: "1px solid var(--gold-border)" }}>
          <div className="flex items-center justify-between" style={{ marginBottom: 16 }}>
            <h3 style={{ fontSize: 14, fontWeight: 600, margin: 0 }}>Nova Aluna</h3>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(180px,1fr))", gap: 12 }}>
            ...
            {form.email && (
              <div style={{ gridColumn: "1 / -1", padding: "14px 16px", background: "rgba(201,168,76,0.06)", border: "1px solid rgba(201,168,76,0.2)", borderRadius: 10 }}>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 12px", background: "rgba(201,168,76,0.1)", border: "1px solid rgba(201,168,76,0.3)", borderRadius: 8 }}>
                  <span style={{ width: 8, height: 8, borderRadius: "50%", background: "#C9A84C", display: "inline-block", flexShrink: 0 }} />
                  <span style={{ fontSize: 12, fontWeight: 700, color: "#C9A84C" }}>Seja Incomum</span>
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: 10, marginBottom: 14, alignItems: "center" }}>
        ...
        />
        {(["todas", "ativo", "pendente", "inativo"] as const).map(f => {
      ...
      {filtradas.length === 0 ? (
        <div className="card" style={{ padding: "48px 20px", textAlign: "center" }}>
          <Crown size={28} style={{ color: "var(--gold)", opacity: 0.3, marginBottom: 12 }} />
          <p style={{ fontSize: 14, color: "var(--text-muted)", margin: 0 }}>{termo ? "Nenhuma aluna encontrada para essa pesquisa." : "Nenhuma aluna cadastrada ainda."}</p>
        </div>
      ) : (
            ...
            return (
              <div key={c.id} className="card" style={{ padding: 0, overflow: "hidden" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 14, padding: "14px 18px", cursor: "pointer" }} onClick={() => setDetalhe(aberto ? null : c)}>
                  <div className="avatar" style={{ width: 38, height: 38, background: "rgba(201,168,76,0.15)", color: "var(--gold)", fontSize: 13, flexShrink: 0 }}>
                    {c.nome.split(" ").map(n => n[0]).join("").slice(0,2)}
                  </div>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <p style={{ fontSize: 14, fontWeight: 600, margin: "0 0 2px" }}>{c.nome}</p>
                    <div className="flex items-center gap-6" style={{ gap: 8 }}>
                  ...
                  <div style={{ width: 70, textAlign: "center" }}>
                    <p style={{ fontSize: 12, fontWeight: 700, color: "var(--gold)", margin: "0 0 3px" }}>{c.progresso}%</p>
                    <div style={{ height: 4, background: "var(--border)", borderRadius: 999 }}>
                      <div style={{ height: "100%", width: `${c.progresso}%`, background: "var(--gold)", borderRadius: 999 }} />
                    </div>
                  </div>
                  ...
                  <div style={{ borderTop: "1px solid var(--border)", padding: "18px 20px", background: "var(--bg)" }}>
                    {editando?.id === c.id ? (
                      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(180px,1fr))", gap: 12 }}>
                        ...
                        <div>
                          <label style={{ fontSize: 11, color: "var(--text-muted)", display: "block", marginBottom: 4 }}>Progresso (%)</label>
                          <input type="number" min={0} max={100} value={editando.progresso} onChange={e => setEditando({ ...editando, progresso: Number(e.target.value) })} style={{ width: "100%", padding: "9px 12px", fontSize: 13 }} />
                        </div>
                        <div>
                          <label style={{ fontSize: 11, color: "var(--text-muted)", display: "block", marginBottom: 4 }}>Status do Pagamento</label>
                      ...
                      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(220px,1fr))", gap: 20 }}>
                        ...
                        <div>
                          <p style={{ fontSize: 11, color: "var(--text-muted)", fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 10 }}>Status de Acesso</p>
                          <div style={{ display: "flex", gap: 6 }}>
                            {(["ativo","pendente","inativo"] as StatusAcesso[]).map(s => {
