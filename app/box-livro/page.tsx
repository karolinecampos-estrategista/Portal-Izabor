...
import {
  ...
  Instagram, TrendingUp, Star, AlertCircle, FileText, PlayCircle,
...
const COMPRADORAS_INICIAL: Compradora[] = [
  { id: 1, nome: "Ana Paula Ferreira",  whatsapp: "11987654321", instagram: "anapaula.empreende",    dataCompra: "2025-04-10", variante: "completo",      statusEntrega: "entregue", nivelEngajamento: "engajada", statusUpsell: "em-conversa", temAcessoBW: true,  notas: "Postou stories do livro. Muito receptiva. Tem salão de beleza." },
  { id: 2, nome: "Fernanda Lima",       whatsapp: "21976543210", instagram: "fernanda_lima_beauty",  dataCompra: "2025-04-15", variante: "completo",      statusEntrega: "entregue", nivelEngajamento: "lendo",    statusUpsell: "nao-abordada", temAcessoBW: false, notas: "Confirmou recebimento. Ainda está no começo do livro." },
  { id: 3, nome: "Juliana Costa",       whatsapp: "31965432109", instagram: "jucosta_confeitaria",   dataCompra: "2025-04-20", variante: "so-livro",      statusEntrega: "entregue", nivelEngajamento: "postou",   statusUpsell: "nao-abordada", temAcessoBW: false, notas: "Fez post no feed com o livro. Confeiteira, ótima fit para mentoria." },
  { id: 4, nome: "Camila Rocha",        whatsapp: "11954321098", instagram: "camila.rocha.moda",     dataCompra: "2025-05-01", variante: "completo",      statusEntrega: "enviado",  nivelEngajamento: "recebeu",  statusUpsell: "nao-abordada", temAcessoBW: false, notas: "Confirmou que está aguardando entrega." },
  { id: 5, nome: "Marina Souza",        whatsapp: "41943210987", instagram: "marinasouza_loja",      dataCompra: "2025-05-10", variante: "devocional-30", statusEntrega: "pendente", nivelEngajamento: "sem-contato", statusUpsell: "nao-abordada", temAcessoBW: false, notas: "" },
  { id: 6, nome: "Beatriz Alves",       whatsapp: "85932109876", instagram: "beatriz.alves.fitness", dataCompra: "2025-05-18", variante: "so-livro",      statusEntrega: "pendente", nivelEngajamento: "sem-contato", statusUpsell: "nao-abordada", temAcessoBW: false, notas: "Personal trainer, indicada pela Ana Paula." },
];
...
export default function BoxLivroCompradoras() {
  const [compradoras, setCompradoras] = useState<Compradora[]>(COMPRADORAS_INICIAL);
  ...
  const nextId = Math.max(...compradoras.map((c) => c.id)) + 1;

  function salvarNova() {
    if (!form.nome.trim()) return;
    setCompradoras((p) => [{ ...form, id: nextId }, ...p]);
    setForm(FORM_VAZIO);
    setShowForm(false);
  }
  ...
  function salvarEdicao() {
    if (!editando) return;
    setCompradoras((p) => p.map((c) => (c.id === editando.id ? editando : c)));
    setDetalhe(editando);
    setEditando(null);
  }

  function atualizarCampo<K extends keyof Compradora>(id: number, campo: K, valor: Compradora[K]) {
    setCompradoras((p) => p.map((c) => (c.id === id ? { ...c, [campo]: valor } : c)));
    if (detalhe?.id === id) setDetalhe((p) => p ? { ...p, [campo]: valor } : p);
  }

  const filtradas = compradoras.filter((c) => {
    if (filtroEntrega !== "todas" && c.statusEntrega !== filtroEntrega) return false;
    if (filtroUpsell !== "todas" && c.statusUpsell !== filtroUpsell) return false;
    return true;
  });

  const totalCompradoras = compradoras.length;
  const aguardandoEntrega = compradoras.filter((c) => c.statusEntrega !== "entregue").length;
  const engajadas = compradoras.filter((c) => ["postou", "engajada"].includes(c.nivelEngajamento)).length;
  const convertidas = compradoras.filter((c) => c.statusUpsell === "convertida").length;
  ...
  return (
    <div style={{ maxWidth: 1100, margin: "0 auto", paddingBottom: 48 }}>
      ...
      {showForm && (
        <div className="card" style={{ padding: 24, marginBottom: 24, border: "1px solid var(--gold-border)" }}>
          ...
          <div style={{ display: "flex", gap: 10, justifyContent: "flex-end", marginTop: 20 }}>
            ...
            <button className="btn-gold" onClick={salvarNova} style={{ fontSize: 13 }}>Salvar Compradora</button>
          </div>
        </div>
      )}
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
        {filtradas.length === 0 && (
          <div className="card" style={{ padding: 32, textAlign: "center" }}>
            <p style={{ color: "var(--text-muted)", fontSize: 14 }}>Nenhuma compradora encontrada com esses filtros.</p>
          </div>
        )}
        {filtradas.map((c) => {
          const varCfg = VARIANTES[c.variante];
          const entregaCfg = ENTREGA_CONFIG[c.statusEntrega];
          const upsellCfg = UPSELL_CONFIG[c.statusUpsell];
            ...
            <div key={c.id} className="card" style={{ padding: 0, overflow: "hidden" }}>
              <div className="box-livro-row" onClick={() => setDetalhe(isOpen ? null : c)}>
                ...
                <div style={{ minWidth: 0 }}>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" }}>
                    ...
                    <span style={{ fontSize: 10, color: "var(--text-muted)", whiteSpace: "nowrap" }}>
                      {new Date(c.dataCompra + "T12:00:00").toLocaleDateString("pt-BR", { day: "2-digit", month: "short" })}
                    </span>
                    {c.temAcessoBW && <span style={{ fontSize: 9, fontWeight: 700, padding: "1px 6px", borderRadius: 4, background: "rgba(201,168,76,0.15)", color: "var(--gold)", whiteSpace: "nowrap" }}>BW</span>}
                  </div>
                  <div className="blr-mobile" style={{ gap: 6, flexWrap: "wrap", marginTop: 5, alignItems: "center" }}>
                    <span style={{ fontSize: 10, fontWeight: 600, padding: "1px 7px", borderRadius: 4, background: entregaCfg.bg, color: entregaCfg.cor }}>{entregaCfg.label}</span>
                    <span style={{ fontSize: 10, fontWeight: 600, padding: "1px 7px", borderRadius: 4, background: upsellCfg.bg, color: upsellCfg.cor }}>{upsellCfg.label}</span>
                  </div>
                </div>
                ...
                <div className="blr-col" style={{ minWidth: 110 }}>
                  <EngajamentoBarra nivel={c.nivelEngajamento} />
                </div>
                  ...
                  {editando?.id === c.id ? (
                    <div>
                      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: 14, marginBottom: 20 }}>
                        ...
                        <div>
                          <label style={{ fontSize: 11, color: "var(--text-muted)", display: "block", marginBottom: 4 }}>WhatsApp</label>
                          <input className="input" value={editando.whatsapp} onChange={(e) => setEditando({ ...editando, whatsapp: e.target.value.replace(/\D/g, "") })} />
                        </div>
                        <div>
                          <label style={{ fontSize: 11, color: "var(--text-muted)", display: "block", marginBottom: 4 }}>Instagram</label>
                          <input className="input" value={editando.instagram} onChange={(e) => setEditando({ ...editando, instagram: e.target.value.replace("@", "") })} />
                        </div>
                        <div>
                          <label style={{ fontSize: 11, color: "var(--text-muted)", display: "block", marginBottom: 4 }}>Status de Entrega</label>
                          <select className="input" value={editando.statusEntrega} onChange={(e) => setEditando({ ...editando, statusEntrega: e.target.value as StatusEntrega })}>
                            {Object.entries(ENTREGA_CONFIG).map(([v, cfg]) => <option key={v} value={v}>{cfg.label}</option>)}
                          </select>
                        </div>
                        <div>
                          <label style={{ fontSize: 11, color: "var(--text-muted)", display: "block", marginBottom: 4 }}>Nível de Engajamento</label>
                          <select className="input" value={editando.nivelEngajamento} onChange={(e) => setEditando({ ...editando, nivelEngajamento: e.target.value as NivelEngajamento })}>
                            {Object.entries(ENGAJAMENTO_CONFIG).map(([v, cfg]) => <option key={v} value={v}>{cfg.label}</option>)}
                          </select>
                        </div>
                        <div>
                          <label style={{ fontSize: 11, color: "var(--text-muted)", display: "block", marginBottom: 4 }}>Status Upsell</label>
                          <select className="input" value={editando.statusUpsell} onChange={(e) => setEditando({ ...editando, statusUpsell: e.target.value as StatusUpsell })}>
                            {Object.entries(UPSELL_CONFIG).map(([v, cfg]) => <option key={v} value={v}>{cfg.label}</option>)}
                          </select>
                        </div>
                        <div style={{ gridColumn: "1 / -1" }}>
                          <label style={{ fontSize: 11, color: "var(--text-muted)", display: "block", marginBottom: 4 }}>Notas</label>
                          <textarea className="input" value={editando.notas} onChange={(e) => setEditando({ ...editando, notas: e.target.value })} rows={3} style={{ resize: "vertical" }} />
                        </div>
                      </div>
                      <div style={{ display: "flex", gap: 10, justifyContent: "flex-end" }}>
                        <button className="btn-ghost" onClick={() => setEditando(null)}>Cancelar</button>
                        <button className="btn-gold" style={{ fontSize: 13 }} onClick={salvarEdicao}>Salvar</button>
                      </div>
                    </div>
                  ) : (
                    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(260px, 1fr))", gap: 24 }}>
                      <div>
                        ...
                        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                          ...
                          <p style={{ fontSize: 12, color: "var(--text-muted)", margin: 0 }}>Compra: {new Date(c.dataCompra + "T12:00:00").toLocaleDateString("pt-BR")}</p>
                        </div>
                          ...
                          <div>
                            ...
                            <div style={{ display: "flex", gap: 8 }}>
                              {(["pendente", "enviado", "entregue"] as StatusEntrega[]).map((s) => {
                                const cfg = ENTREGA_CONFIG[s]; const ativo = c.statusEntrega === s;
                          ...
                          <div>
                            ...
                            <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                              {(["sem-contato", "recebeu", "lendo", "postou", "engajada"] as NivelEngajamento[]).map((s) => {
                                const cfg = ENGAJAMENTO_CONFIG[s]; const ativo = c.nivelEngajamento === s;
                          ...
                          <div>
                            ...
                            <div style={{ display: "flex", gap: 8 }}>
                              {(["nao-abordada", "em-conversa", "convertida"] as StatusUpsell[]).map((s) => {
                                const cfg = UPSELL_CONFIG[s]; const ativo = c.statusUpsell === s;
                      ...
                      <div style={{ gridColumn: "1 / -1", display: "flex", justifyContent: "flex-end", gap: 10, paddingTop: 8, borderTop: "1px solid var(--border)" }}>
                        <button className="btn-ghost" style={{ fontSize: 12 }} onClick={(e) => { e.stopPropagation(); setEditando({ ...c }); }}>Editar dados</button>
                      </div>
                    </div>
                  )}
                </div>
      ...
      {compradoras.some((c) => c.statusEntrega === "entregue" && c.nivelEngajamento === "sem-contato") && (
        <div className="card" style={{ marginTop: 24, padding: 20, border: "1px solid rgba(251,191,36,0.3)", background: "rgba(251,191,36,0.05)" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            {compradoras.filter((c) => c.statusEntrega === "entregue" && c.nivelEngajamento === "sem-contato").map((c) => (
