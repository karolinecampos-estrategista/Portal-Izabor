...
import { Trophy, Plus, X, Users, Clock, Send, Flame, Tag, ChevronDown, ChevronUp, Trash2, Loader2, User } from "lucide-react";
...
const FORM_VAZIO = {
  ...
  prazo: "", destino: "todas-bw" as Destino, mentorada_nome: "",
};
  ...
  async function enviar() {
    ...
    const res = await fetch("/api/desafios", {
      ...
      body: JSON.stringify({
        ...
        mentorada_nome: form.destino === "individual" ? form.mentorada_nome || null : null,
      }),
    });
      ...
      {mostrarForm && (
        <div className="card" style={{ padding: "20px 22px", marginBottom: 24, border: "1px solid var(--gold-border)", background: "linear-gradient(135deg,#111 0%,#130f04 100%)" }}>
          ...
          <div style={{ display: "grid", gap: 12 }}>
            <input value={form.titulo} onChange={(e) => setForm({ ...form, titulo: e.target.value })} style={{ padding: "10px 12px", fontSize: 13, width: "100%" }} placeholder="Título do desafio..." />
            <textarea value={form.descricao} onChange={(e) => setForm({ ...form, descricao: e.target.value })} style={{ padding: "10px 12px", fontSize: 13, width: "100%", minHeight: 80, resize: "vertical" }} placeholder="Descreva o desafio — o que a mentorada deve fazer?" />
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
              <div style={{ flex: 1, minWidth: 120 }}>
                <p style={{ fontSize: 11, color: "var(--text-muted)", marginBottom: 4 }}>Pilar</p>
                <select value={form.pilar} onChange={(e) => setForm({ ...form, pilar: e.target.value as Pilar })} style={{ padding: "9px 12px", fontSize: 13, width: "100%" }}>
              ...
              <div style={{ flex: "0 0 auto" }}>
                ...
                <input type="date" value={form.prazo} onChange={(e) => setForm({ ...form, prazo: e.target.value })} style={{ padding: "9px 12px", fontSize: 13, width: "auto" }} />
              </div>
              <div style={{ flex: 1, minWidth: 130 }}>
                <p style={{ fontSize: 11, color: "var(--text-muted)", marginBottom: 4 }}>Enviar para</p>
                <select value={form.destino} onChange={(e) => setForm({ ...form, destino: e.target.value as Destino })} style={{ padding: "9px 12px", fontSize: 13, width: "100%" }}>
            ...
            <div className="flex items-center gap-3">
              <button className="btn-gold flex items-center gap-2" onClick={enviar} disabled={salvando || !form.titulo.trim() || (form.destino === "individual" && !form.mentorada_nome)}>
                {salvando ? <Loader2 size={13} style={{ animation: "spin 1s linear infinite" }} /> : <Send size={13} />}
                {salvando ? "Enviando..." : "Enviar Desafio"}
              </button>
